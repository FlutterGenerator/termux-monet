                    ...
                    try (ZipInputStream zipInput = new ZipInputStream(zipUrl.openStream())) {
                        ...
                        while ((zipEntry = zipInput.getNextEntry()) != null) {
                            if (zipEntry.getName().equals("SYMLINKS.txt")) {
                                ...
                                File targetFile = new File(TERMUX_STAGING_PREFIX_DIR_PATH, zipEntryName).getCanonicalFile();
                                File stagingDir = new File(TERMUX_STAGING_PREFIX_DIR_PATH).getCanonicalFile();
                                if (!targetFile.getPath().startsWith(stagingDir.getPath())) {
                                    Logger.logError("Skipping invalid entry outside staging dir: " + zipEntryName);
                                    continue;
                                }
